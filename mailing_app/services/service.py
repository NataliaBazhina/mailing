...
def can_send_mailing(mailing: Mailing) -> bool:
    last_attempt = (
        MailingTrying.objects.filter(
            mailing=mailing, status=MailingTrying.status_trying.SUCCESS
        )
...
def send_mailing(mailing: Mailing):
    ...
    attempt_status = MailingTrying.Status.SUCCESS
    ...
    try:
        ...
        mailing.save()
        attempt_status = MailingTrying.Status.FAILED
